...
from app.modules.freight.commercial_pricing import apply_commercial_markup
from app.modules.freight.schemas import PriceRequest

logger = logging.getLogger("freight-api")


def _freight_data_dir() -> Path:
    configured = os.getenv("FREIGHT_DATA_DIR")
    if configured:
        return Path(configured).resolve()
    return Path(__file__).resolve().parents[2] / "data" / "freight"


BASE_DIR = _freight_data_dir()
...
def _ensure_freight_assets() -> None:
    global _model, _df, _assets_loaded
    if _assets_loaded:
        return
    _assets_loaded = True
    try:
        _model = joblib.load(BASE_DIR / "price_model.pkl")
    except Exception as exc:
        logger.warning("Price model could not be loaded: %s", exc)
        _model = None
    _df = load_price_history()


def find_similar_price(req: PriceRequest, distance: float) -> float | None:
    _ensure_freight_assets()
...
def compute_price(req: PriceRequest) -> dict:
...
def probe_freight_data() -> dict:
    _ensure_freight_assets()
    data_dir = BASE_DIR
    model_path = data_dir / "price_model.pkl"
    csv_path = data_dir / "freight_prices.csv"
    fuel_path = data_dir / "fuel_prices.json"
    df = _df if _df is not None else pd.DataFrame()
    return {
        ...
        "fuel_file_exists": fuel_path.is_file(),
        "price_history_rows": len(df),
    }
